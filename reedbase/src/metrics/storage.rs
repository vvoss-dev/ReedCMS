...
mod tests {
    ...
    fn test_format_tags() {
        ...
        let mut metric = Metric::new("test", 1.0, MetricUnit::Count);
        metric.tags.insert("a".to_string(), "1".to_string());
        metric.tags.insert("b".to_string(), "2".to_string());

        let formatted = storage.format_tags(&metric);
