...
# ReedCMS Uninstallation Script
# Removes reed binary and man pages from system
...
echo -e "${BLUE}ReedCMS Uninstallation${NC}"
echo "======================"
echo ""

# Check for sudo if needed
if [ -f /usr/local/bin/reed ] || [ -L /usr/local/bin/reed ]; then
    if [ "$EUID" -ne 0 ]; then
